...
android {
    namespace = "io.github.dkmarkell.textresource.test"
    compileSdk = 36
    ...
    afterEvaluate {
        publishing {
            publications {
                ...
                releasePub.artifactId = "textresource-test"
                releasePub.version = (findProperty("VERSION_NAME") as String?) ?: project.version.toString()

                releasePub.pom {
                    name.set("TextResource Test")
                    description.set(
                        "Robolectric-based test utilities for TextResource. Provides a simple resolver to evaluate string and plural resources with a chosen Locale from local unit tests, keeping Android boilerplate out of your specs. Requires Robolectric in the consuming project; no transitive dependencies are bundled."
                    )
